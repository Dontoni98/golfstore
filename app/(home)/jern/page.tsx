...
interface Product { // Hva vi forventer å få fra api
  productId: number;
  productName: string;
  price: number;
  brandName: string;
...
export default function Jern() {
  ...
  useEffect(() => { //kjøres når react rendrer komponentet for første gang (mount), blir unmounta når vi forlater siden.
    async function fetchData() {
      try {
        // Prøver først å hente data fra api-endepunktet
        const res = await fetch(
          'http://localhost:8080/products/MenuGrid?category=Jern'
        );
        const json: ApiResponse = await res.json(); // parsrer responsen som JSOn
        setProduct(json.content);
      } catch (err) { // ved feil fanges det her
        console.error(err);
  ...
  return (
    <>
      <h1 className="text-3xl font-bold mb-6">Jern</h1>
