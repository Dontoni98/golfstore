...
export default function SearchResults() {
  ...
  useEffect(() => {
    async function fetchResults() { // henter resultater fra backend når man endrer det som søkes på
      if (!query) return; // gjør at det ikke søkes om det ikke er skrevet noe

      try {
        const res = await fetch(
          `http://localhost:8080/products/MenuGrid?search=${encodeURIComponent(query)}`
        );
  ...
  return (
    <div className="p-6">
      ...
      <p className="mb-4">Du søkte etter: <strong>{query}</strong></p>
